...
void ConfigureEPWM(void) {
  ...
  // Setup TB
  //
  EPwm4Regs.TBPRD = PWM_MAX_COUNT;                // Set timer period
  ...
  //// ClkCfgRegs.PERCLKDIVSEL.bit.EPWMCLKDIV = 0;  // EPWMCLK = PLLSYSCLK
  // EPWMCLK already equals to SYSCLK / 2
  EPwm4Regs.TBCTL.bit.HSPCLKDIV = TB_DIV1;  // /1
  ...
  EPwm4Regs.CMPA.bit.CMPA = PWM_MID_COUNT;  // Set compare A value to max/2 counts
  ...
  EPwm4Regs.AQCTLB.bit.CBU = AQ_NO_ACTION;  // B No Action
  EPwm4Regs.AQCTLB.bit.CBD = AQ_NO_ACTION;

  //
  // Setup DB
  //
  EPwm4Regs.DBCTL.bit.IN_MODE = DBA_ALL;          // 只接受AQ出来的EpwmxA
  EPwm4Regs.DBCTL.bit.POLSEL = DB_ACTV_HIC;       // active high complementary
  EPwm4Regs.DBCTL.bit.OUT_MODE = DB_FULL_ENABLE;  // enable s1, s0
...
void changeCMP_value_brige2(float32 val) {
  Uint16 cmp1 = (PWM_MID_COUNT - PWM_MID_COUNT * val);
  Uint16 cmp2 = (PWM_MID_COUNT + PWM_MID_COUNT * val);
  EPwm3Regs.CMPA.bit.CMPA = cmp1;
  EPwm4Regs.CMPA.bit.CMPA = cmp2;
}
